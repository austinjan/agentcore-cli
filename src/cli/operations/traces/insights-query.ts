...
import { OperationTimeoutError, ResourceNotFoundError, toError } from '../../errors';
...
async function pollQueryResults(client: CloudWatchLogsClient, queryId: string): Promise<InsightsQueryResult> {
  ...
  return { success: false, error: new OperationTimeoutError('Query timed out after 60 seconds') };
}
