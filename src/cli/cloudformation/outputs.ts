...
export function buildDeployedState(opts: BuildDeployedStateOptions): DeployedState {
  ...
  // Merge dataset state: CFN outputs provide datasetId/datasetArn; preserve sourceHash from existing state
  if (datasets && Object.keys(datasets).length > 0) {
    const existingDatasets = existingState?.targets?.[targetName]?.resources?.datasets ?? {};
    const mergedDatasets: Record<string, DatasetDeployedState> = {};
    for (const [name, ds] of Object.entries(datasets)) {
      mergedDatasets[name] = {
        ...ds,
        sourceHash: existingDatasets[name]?.sourceHash,
      };
    }
    targetState.resources!.datasets = mergedDatasets;
  }
