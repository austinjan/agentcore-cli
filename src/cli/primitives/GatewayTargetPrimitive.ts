import { APP_DIR, MCP_APP_SUBDIR, findConfigRoot, requireConfigRoot, resultToJson } from '../../lib';
import type {
  AgentCoreCliMcpDefs,
...
import type { AddScreenComponent, Result } from './types';
...
export class GatewayTargetPrimitive extends BasePrimitive<AddGatewayTargetOptions, RemovableGatewayTarget> {
  ...
  async previewRemove(name: string): Promise<RemovalPreview> {
    ...
    const tool = tools.find(t => t.name === name);
    if (!tool) {
      throw new ResourceNotFoundError(`Gateway target "${name}" not found.`);
    }
