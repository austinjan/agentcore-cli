...
export async function handleProjectStatus(
  ...
  if (targetConfig) {
    // Promote the target region to AWS_REGION/AWS_DEFAULT_REGION for any
    // SDK clients constructed without an explicit region inside the block.
...
export async function handleRuntimeLookup(
  ...
  // Promote the target region to AWS_REGION/AWS_DEFAULT_REGION so SDK
  // clients constructed without an explicit region pick it up.
  // See https://github.com/aws/agentcore-cli/issues/924.
