...
export async function handleInvoke(context: InvokeContext, options: InvokeOptions = {}): Promise<InvokeResult> {
  ...
  if (project.runtimes.length === 0) {
    return { success: false, error: new ResourceNotFoundError('No agents defined in configuration') };
  }
  ...
  if (agentSpec.protocol === 'MCP') {
    ...
    if (options.prompt === 'list-tools') {
      try {
        ...
        return {
          success: false,
          error: new Error(`Failed to list MCP tools: ${getErrorMessage(err)}`, { cause: toError(err) }),
        };
      }
    }
    ...
    if (options.prompt === 'call-tool') {
      if (!options.tool) {
        return {
          success: false,
          error: new ValidationError('MCP call-tool requires --tool <name>. Use "list-tools" to see available tools.'),
        };
      }
      ...
      try {
        ...
        return {
          success: false,
          error: new Error(`Failed to call MCP tool: ${getErrorMessage(err)}`, { cause: toError(err) }),
        };
      }
    }
  ...
  if (agentSpec.protocol === 'A2A') {
    try {
      ...
      return {
        success: false,
        error: new Error(`A2A invoke failed: ${getErrorMessage(err)}`, { cause: toError(err) }),
      };
    }
  }
  ...
  if (agentSpec.protocol === 'AGUI') {
    ...
    try {
      ...
      return {
        success: false,
        error: new Error(`AGUI invoke failed: ${getErrorMessage(err)}`, { cause: toError(err) }),
      };
    }
  }
